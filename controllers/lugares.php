...
class Lugares extends Controller {
	...
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("lugares",$limite);
	...
	$this->view->lugaresList = $this->model->lugaresList($start,NUM_ITEMS_BY_PAGE);
	$this->view->render("lugares/index");
	}
	...
	$this->view->lugaresList = $this->model->lugaresListUltimo();
	$this->view->render("lugares/index");
	}
	public function add(){
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("lugares",$limite);
	$this->view->lugaresList = $this->model->lugaresList($start,NUM_ITEMS_BY_PAGE);
	$this->view->lugaresListUltimo = $this->model->lugaresListUltimo();
	$this->view->render("lugares/add");
	}
	public function search(){
	$start=0;
	$this->view->page=1;
	$this->view->lugaresList = $this->model->lugaresSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->lugaresList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("lugares/index");
	}
	...
	$limite=10;
	$this->view->anyList=$this->model->anyList("lugares",$limite);
	$this->view->lugaresList = $this->model->lugaresSingleList($id);
	$this->view->render("lugares/view");
	}
    public function edit($id){
		$limite=10;
		$this->view->anyList=$this->model->anyList("lugares",$limite);
		$this->view->lugaresList = $this->model->lugaresSingleList($id);
		$this->view->render("lugares/edit");
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."lugares/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."lugares/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."lugares/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."lugares/delete/$id  id=erase>
