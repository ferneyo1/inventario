...
class Marcas extends Controller {
	...
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("marcas",$limite);
	...
	$this->view->marcasList = $this->model->marcasList($start,NUM_ITEMS_BY_PAGE);
	...
	$this->view->marcasList = $this->model->marcasListUltimo();
	$this->view->render("marcas/index");
	}
	public function add(){
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("marcas",$limite);
	$this->view->marcasList = $this->model->marcasList($start,NUM_ITEMS_BY_PAGE);
	$this->view->marcasListUltimo = $this->model->marcasListUltimo();
	$this->view->render("marcas/add");
	}
	public function search(){
	$start=0;
	$this->view->page=1;
	$this->view->marcasList = $this->model->marcasSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->marcasList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("marcas/index");
	}
	...
	$limite=10;
	$this->view->anyList=$this->model->anyList("marcas",$limite);
	$this->view->marcasList = $this->model->marcasSingleList($id);
	$this->view->render("marcas/view");
	}
    public function edit($id){
		$limite=10;
		$this->view->anyList=$this->model->anyList("marcas",$limite);
		$this->view->marcasList = $this->model->marcasSingleList($id);
		$this->view->render("marcas/edit");
	...
	//salida de datos como una tabla
        ...
        //header("location: " . URL . "marcas/add");
}
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."marcas/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."marcas/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."marcas/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."marcas/delete/$id  id=erase>
