...
class Memorias extends Controller {
	...
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("memorias",$limite);
	...
	$this->view->memoriasList = $this->model->memoriasList($start,NUM_ITEMS_BY_PAGE);
	$this->view->render("memorias/index");
	}
	...
	$this->view->memoriasList = $this->model->memoriasListUltimo();
	$this->view->render("memorias/index");
	}
	public function add(){
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("memorias",$limite);
	$this->view->memoriasList = $this->model->memoriasList($start,NUM_ITEMS_BY_PAGE);
	$this->view->memoriasListUltimo = $this->model->memoriasListUltimo();
	$this->view->render("memorias/add");
	}
	public function search(){
	$start=0;
	$this->view->page=1;
	$this->view->memoriasList = $this->model->memoriasSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->memoriasList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("memorias/index");
	}
	...
	$limite=10;
	$this->view->anyList=$this->model->anyList("memorias",$limite);
	$this->view->memoriasList = $this->model->memoriasSingleList($id);
	$this->view->render("memorias/view");
	}
    public function edit($id){
		$limite=10;
		$this->view->anyList=$this->model->anyList("memorias",$limite);
		$this->view->memoriasList = $this->model->memoriasSingleList($id);
		$this->view->render("memorias/edit");
	...
	public function copy($id){
		...
		$this->view->anyList=$this->model->anyList("memorias",$limite);
		$this->view->memoriasList = $this->model->memoriasSingleList($id);
		$this->view->render("memorias/copy");
	...
	//salida de datos como una tabla
        ...
        //header("location: " . URL . "memorias/add");
}
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."memorias/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."memorias/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."memorias/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."memorias/delete/$id  id=erase>
