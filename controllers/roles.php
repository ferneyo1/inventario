...
class Roles extends Controller {
	...
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("roles",$limite);
	...
	$this->view->rolesList = $this->model->rolesList($start,NUM_ITEMS_BY_PAGE);
	...
	$this->view->rolesList = $this->model->rolesListUltimo();
	$this->view->render("roles/index");
	}
	public function add(){
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("roles",$limite);
	$this->view->rolesList = $this->model->rolesList($start,NUM_ITEMS_BY_PAGE);
	$this->view->rolesListUltimo = $this->model->rolesListUltimo();
	$this->view->render("roles/add");
	}
	public function search(){
	$start=0;
	$this->view->page=1;
	$this->view->rolesList = $this->model->rolesSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->rolesList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("roles/index");
	}
	...
	$limite=10;
	$this->view->anyList=$this->model->anyList("roles",$limite);
	$this->view->rolesList = $this->model->rolesSingleList($id);
	$this->view->render("roles/view");
	}
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."roles/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."roles/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."roles/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."roles/delete/$id  id=erase>
