...
class Usuarios extends Controller {
	...
	public function index(){$this->view->rolesList = $this->model->rolesList();
	$start=0;
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("usuarios",$limite);
	...
	$this->view->usuariosList = $this->model->usuariosList($start,NUM_ITEMS_BY_PAGE);
	$this->view->render("usuarios/index");
	}
	public function index_($page){$this->view->rolesList = $this->model->rolesList();
	if(isset($page)){
		$this->view->page=$page;
	}
	if(!$page){
	...
	public function ultimo(){$this->view->rolesList = $this->model->rolesList();
	$this->view->usuariosList = $this->model->usuariosListUltimo();
	$this->view->render("usuarios/index");
	}
	public function add(){$this->view->rolesList = $this->model->rolesList();
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("usuarios",$limite);
	$this->view->usuariosList = $this->model->usuariosList($start,NUM_ITEMS_BY_PAGE);
	$this->view->usuariosListUltimo = $this->model->usuariosListUltimo();
	$this->view->render("usuarios/add");
	}
	public function search(){$this->view->rolesList = $this->model->rolesList();
	$start=0;
	$this->view->page=1;
	$this->view->usuariosList = $this->model->usuariosSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->usuariosList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("usuarios/index");
	}
	public function view($id){$this->view->rolesList = $this->model->rolesList();
	$limite=10;
	$this->view->anyList=$this->model->anyList("usuarios",$limite);
	$this->view->usuariosList = $this->model->usuariosSingleList($id);
	$this->view->render("usuarios/view");
	}
    public function edit($id){$this->view->rolesList = $this->model->rolesList();
		$limite=10;
		$this->view->anyList=$this->model->anyList("usuarios",$limite);
		$this->view->usuariosList = $this->model->usuariosSingleList($id);
	...
	public function copy($id){$this->view->rolesList = $this->model->rolesList();
		$limite=1;
		$this->view->anyList=$this->model->anyList("usuarios",$limite);
		$this->view->usuariosList = $this->model->usuariosSingleList($id);
		$this->view->render("usuarios/copy");
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."usuarios/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."usuarios/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."usuarios/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."usuarios/delete/$id  id=erase>
