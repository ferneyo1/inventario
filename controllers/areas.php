...
class Areas extends Controller {
	...
	$this->view->page=1;
	$limite="1000000";
	$this->view->totalList=$this->model->anyList("areas",$limite);
	...
	$this->view->areasList = $this->model->areasList($start,NUM_ITEMS_BY_PAGE);
	...
	$this->view->areasList = $this->model->areasListUltimo();
	$this->view->render("areas/index");
	}
	public function add(){
	$limite=10;
	$start=0;
	$this->view->anyList=$this->model->anyList("areas",$limite);
	$this->view->areasList = $this->model->areasList($start,NUM_ITEMS_BY_PAGE);
	$this->view->areasListUltimo = $this->model->areasListUltimo();
	$this->view->render("areas/add");
	}
	public function search(){
	$start=0;
	$this->view->page=1;
	$this->view->areasList = $this->model->areasSingleSearch($_POST["dato"],$_POST["criterio"]);
	$this->view->total_registros=count($this->view->areasList );
	$this->view->total_pages=ceil($this->view->total_registros/NUM_ITEMS_BY_PAGE);
	$this->view->render("areas/index");
	}
	...
	$limite=10;
	$this->view->anyList=$this->model->anyList("areas",$limite);
	$this->view->areasList = $this->model->areasSingleList($id);
	$this->view->render("areas/view");
	}
    public function edit($id){
		$limite=10;
		$this->view->anyList=$this->model->anyList("areas",$limite);
		$this->view->areasList = $this->model->areasSingleList($id);
		$this->view->render("areas/edit");
	...
	public function roles($rol,$id,$opcion){
		switch($rol){
			case 1:
				...
				return "
				<a href=".URL."areas/copy/$id>
					<i class='fa fa-repeat'title=Copiar></i>
				</a>&nbsp;
				<a href=".URL."areas/view/$id>
					<i class='fa fa-eye'title=Ver ></i>
				</a>&nbsp;
				<a href=".URL."areas/edit/$id>
					<i class='fa fa-edit'title=Editar ></i>
				</a>&nbsp;
				<a href=".URL."areas/delete/$id  id=erase>
